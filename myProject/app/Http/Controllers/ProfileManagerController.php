...
class ProfileManagerController extends Controller
{
    ...
    function index()
    {
        ...
        return view('profileManager.index',compact(['user','userDonate','userObject']));
    }

    function editProfile(){
        $user = Auth::user();
        
        return view('profileManager.edit',compact(['user']));
    }
    ...
    function statusDonate($id){
        ...
        return view('profileManager.statusDonate',compact(['user','statusDonate']));
    }

    function statusObject($id){
        $user = Auth::user();
        $statusObject = ObjectRequest::getAllByCampaingIdAndUserId($id,$user->id);
        return view('profileManager.statusObject',compact(['user','statusObject']));
    }
